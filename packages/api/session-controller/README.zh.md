...
Cold follow opening 会读取有界 persistence window 与 cached projection baseline，而不构造完整 Session observation。JSONL／Zstandard 仍校验完整物理前缀，但只保留配置的逻辑事件 suffix。因此，打开或重连 cold conversation 既不会展开其完整日志，也不会恢复 Agent；后续 prompt 或其他会激活 Agent 的操作才执行恢复，既有 follower 会衔接新建 Session 的后缀。
...
| `historyPageMaxEvents` | `20,000` | 在按消息对齐选择后，一页历史或 follow opening 最多保留的逻辑事件数 |
