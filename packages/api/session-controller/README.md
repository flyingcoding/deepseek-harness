...
A cold follow opening reads a bounded persistence window and cached projection baseline without constructing a complete Session observation. JSONL/Zstandard scans still validate the complete physical prefix, but retain only the configured logical-event suffix. Opening or reconnecting a cold conversation therefore neither expands its full log nor resumes an Agent; a later prompt or another activating operation performs the resume, and the existing follower bridges the created Session's suffix.
...
| `historyPageMaxEvents` | `20,000` | Maximum logical events retained in one history page or follow opening, after message-aligned selection |
