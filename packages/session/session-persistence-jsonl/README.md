...
`readWindow(id, beforeSeq, maxEvents)` validates every complete row while retaining only the requested backwards window; it does not open a full-log handle or populate the parsed-log memo. A requested complete-prefix visitor traverses the same input bytes again after the exact fork cut is known, retaining one additional event. The logical-event ceiling does not bound the compressed input bytes, one event's payload, or caller-owned projection state. Reading a supported historical generation still performs its complete format migration before bounded current-generation reads.
