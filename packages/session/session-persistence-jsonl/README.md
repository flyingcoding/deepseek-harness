...
| `coldLogMemoMaxEvents` | `1,000,000` | Logical-event ceiling across parsed logs retained for handoff reuse |
...
For current-format input, `readWindow(id, beforeSeq, maxEvents)` validates every complete row while retaining only the requested backwards window; it does not open a full-log handle or populate the parsed-log memo. A requested complete-prefix visitor traverses the same input bytes again after the exact fork cut is known, retaining one additional event. The logical-event ceiling does not bound the compressed input bytes, one event's payload, or caller-owned projection state. Historical input uses the ordinary read-handle restore, which materializes the current logical log without publishing a successor.
