...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  /** Validate the complete stored prefix while retaining only one backwards event window. */
  override async readWindow(
    ...
    if (selected.sourceVersion === SESSION_FORMAT_VERSION) {
      bytes = (await readStableJsonlFile(path, signal)).bytes
    } else {
      const current = await this.ensureCurrentLog(id, signal, selected)
      /* v8 ignore next -- a supplied generation cannot resolve as absent. */
      if (current === undefined) throw new SessionPersistenceNotFoundError(id)
      path = current.path
      bytes = current.snapshot.bytes
    }
