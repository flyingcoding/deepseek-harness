...
| `inspect(id, signal?): Promise<{ meta; events }>` | Return an upgraded, validated, deeply frozen logical view without committing recovery or publishing a Session. A cold view receives in-memory synthetic recovery closers while its physical torn tail remains untouched; an already-live view is its current immutable snapshot and may contain an open turn. Coordinator-backed implementations may retain the cold unpublished Session for later `prepare`; both retained entry count and total logical events are bounded, and stored revision changes discard the retained source. Same-id inspections share an in-flight read. |
