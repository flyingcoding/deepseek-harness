...
describe('SessionPreparations borrowing', () => {
  it('pins an oversized preparation until its observation lease is released', async () => {
    const preparations = new SessionPreparations<PreparedSource, string>(5, 1)
    const source = prepared('borrow-weighted-oversized', 2)

    const lease = await preparations.borrow(source.session.id, () => Promise.resolve(source))
    expect(preparations.has(source.session.id)).toBe(true)
