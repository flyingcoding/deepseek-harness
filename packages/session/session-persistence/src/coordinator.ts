...
/** A backend-provided bounded stored window. */
export interface StoredWindow extends SessionStorageMetadata {
  events: SessionEvent[]
...
export interface PersistenceBackend<TornMarker = unknown> {
  ...
  /**
   * Optional bounded window read for sequential backends that can validate a
   * complete prefix while retaining only the requested logical events.
   */
  loadStoredWindow?(
    id: SessionId,
  ...
  readWindow(
    ...
    return waited.then(() => this.serialize(id, async () => {
      ...
      if (stored === undefined) {
        ...
        const cursor: SessionSeqCursor = whole.events.at(-1)?.seq ?? -1
