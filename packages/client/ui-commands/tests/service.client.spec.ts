...
describe('registration', () => {
  it('registers the "/" source without eager Agent-bound discovery and removes it on fiber disposal', async () => {
    ...
    expect('warm' in source).toBe(false)
    expect([...registered.keys()]).toEqual(['/ command'])
    await fiber.dispose()
    expect(registered.size).toBe(0)
  })

  it('fetches commands when the user requests candidates', async () => {
