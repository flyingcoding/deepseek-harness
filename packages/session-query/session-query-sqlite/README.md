...
| `persistedInspectConcurrency` | `1` | Maximum concurrent persisted-log inspections for inherited batch reads; must be a positive safe integer. Defaults to 1 so one batch read holds at most one inspected log in memory. |

## Tokenizer and limits

The index uses FTS5 `unicode61` over CJK-segmented text: every CJK run is stored as a unigram+bigram token stream, so Chinese and Japanese substrings of any length (including single characters) are searchable, and queries expand their CJK runs into the same bigrams. Latin text keeps token recall rather than arbitrary substring recall: `AI` does not match the token `BRAID`. Use `ctx.sessionQuery.filterEvents()` with a `text` clause when a literal whitespace-flexible substring scan is required. NUL is rejected in queries; reserved highlight markers, caller zero-width separators, and NUL in documents are normalized before indexing so presentation markers cannot collide with source text.
...
- **Latin token recall, not arbitrary substrings** — beyond the CJK unigram+bigram segmentation, the `unicode61` tokenizer does not match substrings inside a larger latin token; use `filterEvents()` for literal scans.
