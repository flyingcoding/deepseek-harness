...
import SessionQueryEngine, {
  ...
  SESSION_QUERY_DEFAULT_PREPARED_SESSION_CACHE_MAX_EVENTS,
...
export interface Config extends SessionQueryConfig {
  ...
  snippetChars?: number
  /** Maximum concurrent persisted-log reads in one inherited batch read. Defaults to 1. */
  ...
  /** Maximum logical events retained by inherited cold observations. Defaults to 1,000,000. */
...
export class SqliteSessionQueryEngine extends SessionQueryEngine {
  ...
  static Config: z<Config> = z.object({
    ...
    preparedSessionCacheMaxEvents: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER)
      .default(SESSION_QUERY_DEFAULT_PREPARED_SESSION_CACHE_MAX_EVENTS),
  })
...
function resolveConfig(config: Config): ResolvedConfig {
  const resolved: ResolvedConfig = {
    ...
    preparedSessionCacheSize: config.preparedSessionCacheSize
      ?? SESSION_QUERY_DEFAULT_PREPARED_SESSION_CACHE_SIZE,
    preparedSessionCacheMaxEvents: config.preparedSessionCacheMaxEvents
      ?? SESSION_QUERY_DEFAULT_PREPARED_SESSION_CACHE_MAX_EVENTS,
  }
  ...
  if (!Number.isSafeInteger(resolved.preparedSessionCacheMaxEvents) || resolved.preparedSessionCacheMaxEvents < 1) {
    throw invalidConfig('preparedSessionCacheMaxEvents must be a positive safe integer')
  }
