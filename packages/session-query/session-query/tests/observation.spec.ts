...
describe('SessionObservationReader cold path', () => {
  ...
  it('releases an oversized observation after its last lease while keeping active readers valid', async () => {
    const ctx = await readerContext()
    try {
      const meta = header('event-budget')
      const store = new Map([[meta.id, {
        header: meta, events: [messageEvent(0, 'first'), messageEvent(1, 'second')], revision: 'r1',
      }]])
      const counters = { stat: 0, open: 0, read: 0 }
      ctx.provide('sessionPersistence', stubPersistence(store, counters))
      const reader = new SessionObservationReader(ctx, 5, 1)
      const first = await reader.read(meta.id, { projectionMode: 'none' })
      const retained = first.retain()
      first[Symbol.dispose]()
      const hit = await reader.read(meta.id, { projectionMode: 'none' })
      expect(hit.events).toBe(retained.events)
      expect(counters.read).toBe(1)
      hit[Symbol.dispose]()
      retained[Symbol.dispose]()
      using fresh = await reader.read(meta.id, { projectionMode: 'none' })
      expect(fresh.events).toEqual(first.events)
