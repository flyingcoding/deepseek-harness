...
export class SessionObservationReader {
   ...
   * @param cacheMaxEvents - maximum logical events retained across unpinned observations.
   */
  constructor(
    private readonly ctx: Context,
    private readonly cacheCapacity: number = SESSION_QUERY_DEFAULT_PREPARED_SESSION_CACHE_SIZE,
    private readonly cacheMaxEvents: number = SESSION_QUERY_DEFAULT_PREPARED_SESSION_CACHE_MAX_EVENTS,
  ) {}
  ...
  private evictPastCapacity(keep?: PreparedEntry): void {
    ...
    for (const entry of this.cache.values()) retainedEvents += entry.events.length
    if (this.cache.size <= this.cacheCapacity && retainedEvents <= this.cacheMaxEvents) return
    for (const [id, candidate] of this.cache) {
      if (candidate === keep || candidate.refs > 0) continue
      this.cache.delete(id)
      retainedEvents -= candidate.events.length
      if (this.cache.size <= this.cacheCapacity && retainedEvents <= this.cacheMaxEvents) return
    }
  }
