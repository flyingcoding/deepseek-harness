...
export abstract class SessionQueryEngine extends Service {
  ...
  constructor(ctx: Context, config: Config = {}) {
    ...
    const preparedSessionCacheMaxEvents = config.preparedSessionCacheMaxEvents
      ?? SESSION_QUERY_DEFAULT_PREPARED_SESSION_CACHE_MAX_EVENTS
    if (!Number.isSafeInteger(preparedSessionCacheMaxEvents) || preparedSessionCacheMaxEvents < 1) {
      throw new SessionQueryError(
        'session-query: preparedSessionCacheMaxEvents must be a positive safe integer',
        'SESSION_QUERY_INVALID_CONFIG',
      )
    }
    this._corpus = new SessionCorpus(ctx, persistedReadConcurrency)
    this._observations = new SessionObservationReader(ctx, preparedSessionCacheSize, preparedSessionCacheMaxEvents)
  }
