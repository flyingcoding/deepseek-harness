...
查询会先去除首尾空白并规范化内部空白，再拆分为词条；每个词条作为一个带引号的字面 FTS5 短语做 AND 匹配，嵌入的引号在绑定前写成两个，因此 `OR`、`NEAR`、引号、括号与 `*` 等 MATCH 运算符会作为数据，而不是可执行的查询语法。系统会在 SQLite 执行前拒绝 NUL。文档中的保留高亮非字符、调用方零宽分隔符与 NUL 会在索引前规范化，因此插入的呈现标记不会与源文本冲突。拉丁匹配遵循分词器 token，而不是任意子串；中文匹配使用[增量索引决策](../../implemented/bug-fix/2026-08-16-session-search-cjk-incremental-index.zh.md)中的 unigram+bigram 分词。

## 分词器选择

持久化 FTS5 表与实时 FTS5 表都使用 `unicode61`。实现实验表明，该分词器支持由两个字符组成的 token `AI`，生成的索引体积约为 trigram 方案的 1/2.1。系统接受的限制是拉丁 token 召回：`AI` 不会匹配较长的 token `BRAID`，任意子串搜索改用提供方无关的文本扫描。[增量索引决策](../../implemented/bug-fix/2026-08-16-session-search-cjk-incremental-index.zh.md)后来在该分词器上叠加了中文 unigram+bigram 分词，因为未拆分的中文连续段让中文查询完全不可搜索；trigram 方案仍因同样的短查询原因在该决策中被否决。
...
选定的分词器以较小的索引体积支持短 token，子串召回仅通过增量索引决策中的中文分词提供。AND 形式的字面词条使查询语法安全且可预测，代价是不支持布尔表达式或完整 MATCH 表达式。取消在操作排队期间会及时生效，在等待数据源期间则会等待其完全停稳；同步 SQLite 执行仍是不可抢占区段。
