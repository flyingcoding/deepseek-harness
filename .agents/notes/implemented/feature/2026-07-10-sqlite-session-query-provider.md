...
Queries are trimmed, whitespace-normalized, and split into terms; each term is ANDed as one quoted literal FTS5 phrase with embedded quotes doubled, so MATCH operators such as `OR`, `NEAR`, quotes, parentheses, and `*` remain data rather than executable query syntax. NUL is rejected before SQLite execution. Reserved highlight noncharacters, caller zero-width separators, and NUL in documents are normalized before indexing, making inserted presentation markers collision-free. Latin matching follows tokenizer tokens rather than arbitrary substrings; CJK matching uses the unigram+bigram segmentation of the [incremental-index decision](../../implemented/bug-fix/2026-08-16-session-search-cjk-incremental-index.md).

## Tokenizer choice

Both persistent and live FTS5 tables use `unicode61`. The implementation experiment found that this tokenizer supports the two-character token `AI` and produces an index about 2.1× smaller than the trigram alternative. The accepted limitation is latin token recall: `AI` does not match the larger token `BRAID`, and arbitrary substring search uses the provider-independent text scan instead. The [incremental-index decision](../../implemented/bug-fix/2026-08-16-session-search-cjk-incremental-index.md) later layered CJK unigram+bigram segmentation over this tokenizer because unsplit CJK runs made Chinese queries unsearchable; the trigram alternative remains rejected there for the same short-query reason.
