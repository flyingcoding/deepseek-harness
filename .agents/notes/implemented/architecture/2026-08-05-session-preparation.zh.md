...
`inspect(id, signal?)` 不修改存储。合成 closer 只存在于准备完成的内存视图中，撕裂的物理尾部保持不变。同 id 调用方共享进行中的冷读。准备完成后，该对象可以进入每个协调器自己的 LRU；其条目容量默认为五，总逻辑事件容量默认为 20,000，第一方后端可配置两者。LRU 会淘汰最早的就绪条目，直至两个限制都满足，因此单个超限检查仍会成功，但不会被保留。协调器复用保留源之前会读取该 id 的当前 revision；如果不匹配，就淘汰处于就绪阶段的源并重新完成冷实体化。已经进入提交或为恢复而预留的源仍由其所有者独占，因此并发检查会借用该不可变视图，直至发布或释放。
