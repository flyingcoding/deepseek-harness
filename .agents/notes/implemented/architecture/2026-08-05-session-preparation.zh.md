...
`inspect(id, signal?)` 不修改存储。合成 closer 只存在于准备完成的内存视图中，撕裂的物理尾部保持不变。同 id 调用方共享进行中的冷读。准备完成后，该对象可以进入每个协调器自己的 LRU；其条目容量默认为五，总逻辑事件容量默认为 1,000,000，第一方后端可配置两者。LRU 会淘汰最早的就绪条目，直至两个限制都满足，因此单个超限检查仍会成功，但不会被保留。协调器复用保留源之前会读取该 id 的当前 revision；如果不匹配，就淘汰处于就绪阶段的源并重新完成冷实体化。已经进入提交或为恢复而预留的源仍由其所有者独占，因此并发检查会借用该不可变视图，直至发布或释放。
...
历史读取使用 `inspect()`，因此只要准备状态符合 LRU 限制，重复分页就可以借用同一份不可变状态，而不会激活 agent。后续恢复会直接取得该保留 Session。超限 Session 的冷 Agent 解析不会预先检查：Host resolver 先列出 header，再调用一次 `prepare()`，最后从该未发布 Session 构建作用域设置，因此 ready LRU 拒绝保留它也不会让一次恢复执行两次完整实体化。
...
冷 continuable subagent 访问先通过 `readFrom()` 只读取子会话的 descriptor 后缀，再由 `ctx.agents.resume()` 准备并发布完整 Session。顺序后端可能为脱离的后缀解析一次产物、为恢复再解析一次，但授权结果只保留返回的小型后缀，绝不会同时保留第二份完整逻辑 Session。该路径继续遵循 [continuable subagent 会话决策](../feature/2026-07-28-continuable-subagent-conversations.zh.md)中的生命周期与授权规则。
...
- 缓存属于单个持久化协调器，而不是进程全局 Session map。实时 Session 由现有存储持有，绝不占用准备条目或事件容量。
...
共享持久化约定规定冷检查不得修改存储且须保持配平，并覆盖后续修复。`persistence.spec.ts` 与 `preparations.spec.ts` 覆盖同 id 进行中读取共享、检查与准备之间的精确 Session 复用、在历史读取与恢复前由 revision 触发刷新、修复只提交一次、独占预留、设置失败后释放、按条目数和逻辑事件数执行 LRU 淘汰、预留期间拒绝 append，以及只允许发布预留 Session。后端测试覆盖完整读取与轻量读取使用同一 revision 身份。API Remote 与 Host 测试覆盖单次实体化的冷设置；continuable subagent 测试覆盖统一发布流水线前的仅后缀授权，包括取消和清理路径。
...
一次受限的冷实体化可以同时服务历史分页、subagent descriptor 检查和后续恢复。所有权转移去除了恢复阶段的冗余复制；每个协调器同时受条目数和事件数限制的 LRU 会阻止少量数百万事件 Session 填满 V8 heap，也避免查询创建实时 agent。新建和恢复共享同一发布协议，同时保持 agent 与 Session 职责分离。

首次冷检查需要承担完整验证与 Session 构造成本。符合两个限制的视图可以保留到被淘汰；超限视图会在读取方释放后回收，因此重复冷分页会再次承担读取成本。持久化层必须协调预留、append、修复和发布；调用方必须把检查结果视为借用的不可变状态。依赖默认 `prepare()` 的后端仍然正确，但无法获得复用优化。
