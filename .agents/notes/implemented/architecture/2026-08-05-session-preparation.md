...
`inspect(id, signal?)` does not mutate storage. Synthetic closers exist only in the prepared in-memory view, and a torn physical tail remains untouched. Same-id callers share an in-flight cold read. Once ready, the preparation may remain in a per-coordinator LRU whose entry capacity defaults to five and whose total logical-event capacity defaults to 1,000,000; first-party backends configure both. The LRU evicts oldest ready entries until both limits hold, so a single oversized inspection succeeds but is not retained. Before reusing a retained source, the coordinator reads that id's current revision; a mismatch evicts a ready source and repeats the cold materialization. A source already committing or reserved for resume remains exclusively owned, so concurrent inspection borrows that immutable view until publication or release.
...
History reads use `inspect()`, so repeated pages borrow the same immutable prepared state without activating an Agent while that state fits the LRU limits. A later resume receives the exact retained Session. An oversized cold Agent resolution does not pre-inspect: Host resolvers list its header, call `prepare()` once, then build scoped setup from that unpublished Session, so rejecting it from the ready LRU does not cause two complete materializations in one resume.
...
Cold continuable-subagent access reads only the child's descriptor suffix through `readFrom()` before `ctx.agents.resume()` prepares and publishes the complete Session. A sequential backend may parse the artifact once for the detached suffix and again for resume, but the authorization result retains only the small returned suffix, never a second complete logical Session. This preserves the lifecycle and authorization rules in the [continuable subagent conversation decision](../feature/2026-07-28-continuable-subagent-conversations.md).
...
- The cache belongs to one persistence coordinator, not a process-global Session map. Live Sessions are owned by the existing stores and never occupy preparation entry or event capacity.
...
The shared persistence contract pins non-mutating balanced cold inspection and later repair. `persistence.spec.ts` and `preparations.spec.ts` pin same-id in-flight sharing, exact Session reuse across inspect and prepare, revision-triggered refresh before history and resume, single repair commit, exclusive reservation, release after failed setup, entry-count and logical-event LRU eviction, append rejection during reservation, and publication of only the reserved Session. Backend tests pin that full and lightweight reads use the same revision identity. API Remote and Host tests pin one-materialization cold setup; continuable-subagent tests pin suffix-only authorization before the common publication pipeline, including cancellation and teardown.
...
One bounded cold materialization can serve history pagination, subagent descriptor inspection, and a later resume. Ownership transfer removes redundant restoration clones, while the entry-and-event-bounded per-coordinator LRU prevents a few multi-million-event Sessions from filling the V8 heap and avoids creating live Agents for queries. Create and resume share one publication protocol without merging Agent and Session responsibilities.

The first cold inspection pays the complete validation and Session-construction cost. A view within both limits may remain until eviction; an oversized view is released after its reader drops it, so repeated cold pages repay the read cost. Persistence must coordinate reservation, append, repair, and publication, and callers must treat inspection values as immutable borrowed state. Backends that rely on the default `prepare()` remain correct but do not receive the reuse optimization.
