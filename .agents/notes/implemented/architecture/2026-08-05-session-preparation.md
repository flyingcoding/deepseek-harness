...
The session-query cache bounds both entry count and retained logical events, and only evicts unpinned entries. JSONL's parsed-log memo has a separate event budget. Counting Sessions alone cannot constrain memory when one stored log contains millions of events; active read leases retain their exact cut until release.
