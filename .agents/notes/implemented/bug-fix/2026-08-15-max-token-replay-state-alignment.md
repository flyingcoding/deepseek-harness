...
The released-v0 migration wraps a known flat pi-ai version-1 state intact as opaque `response` metadata on both the finish chunk and the assembled message. It does not invent version-2 signatures or remove the private payload. The same provider-neutral fallback therefore remains available after Session format migration, and the historical source generation stays unchanged.
