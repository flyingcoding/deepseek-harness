...
已发布 v0 的迁移把已知扁平 pi-ai version-1 状态完整包装为不透明 `response` 元数据，同时更新 finish chunk 与已组装消息。迁移不伪造 version-2 签名，也不删除私有载荷，因此 Session 格式迁移后仍可使用相同的提供方中立回退，历史源 generation 保持不变。
